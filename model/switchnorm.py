...
class SwitchNormalization(Layer):
    ...
    def __init__(self,
        ...
        self.mean_weights_initializer = initializers.get(
        ...
        self.mean_weights_regularizer = regularizers.get(
        ...
        self.mean_weights_constraints = constraints.get(
    ...
    def call(self, inputs, training=None):
        ...
        variance_layer = K.mean(
            temp, self.axis, keepdims=True) - K.square(mean_layer)

        def training_phase():
            mean_batch = K.mean(mean_instance, axis=0, keepdims=True)
            variance_batch = K.mean(
                temp, axis=0, keepdims=True) - K.square(mean_batch)
