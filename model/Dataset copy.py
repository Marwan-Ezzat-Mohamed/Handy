...
class Dataset:
    ...
    def regulate_videos_total_frames(videos_paths, num_frames_per_video, output_folder_name):
        inputs = [(video_path, num_frames_per_video)
    ...
    @staticmethod
    def adjust_video_total_frames_using_interpolation(video_path, num_frames):

        output_path = video_path.replace('.mp4', '_regulated.mp4')
        # read the video
        ...
        if total_frames == num_frames:
            return
        print(
            f'Adjusting video total frames:{total_frames} for video: ' + video_path)
        # calculate the ratio
        ...
        # rename the new video to the original video name
        os.rename(output_path, output_path.replace('_regulated', ''))
    ...
    def rotate_video(input_path, vid_num="", output_folder_name=""):
        ...
        print(f'Rotating video: {input_path}')
    ...
    def change_video_aspect_ratio(input_path, vid_num=1, output_folder_name=""):
       ...
       # create output folder if it does not exist
        ...
        print(
            f'Changing aspect ratio for video:{output_folder} ')
...
def reset_state():
    ...
    shutil.copytree('top_10', 'all_vids')
